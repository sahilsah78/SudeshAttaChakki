...
export function ProgressBar({ axis: setAxis }) {
  ...
  function moveThumb(value) {
    ...
    function applyStyle(value) {
      ...
      progressBarEl.current.style.transform = `scaleX(calc(${value}/4))`;
    }
  }
  ...
  return (
    <div
      ...
      className={`cursor-pointer ${isAxisX ? "touch-pan-y" : "touch-pan-x"} py-3 select-none active:cursor-grabbing`}
    >
      <div className="relative flex h-1 w-full items-center bg-gray-400 *:pointer-events-none">
        <span
          ref={progressBarEl}
          className="inline-block h-full w-1 origin-[left] bg-white"
        ></span>
        {/* chant the arranements if possible html shoudl it self make the bar big and small. */}
        <button
          ref={thumbEl}
          className="absolute size-4 rounded-full bg-white shadow-2xl duration-0 -translate-x-1/2 "
        ></button>
      </div>
    </div>
  );
